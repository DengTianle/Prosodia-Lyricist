"""Generate lyric lines from a melody MIDI using a trained Prosodia checkpoint."""
...
def infer(
    ...
    *,
    ...
    if temperature <= 0 or top_k < 1 or max_new_tokens < 1:
    ...
    if max_new_tokens >= model.bart.config.max_position_embeddings:
        raise ValueError("max_new_tokens must be smaller than the checkpoint's position limit")
    ...
    for record in records:
        encoded = encode_source(record, tokenizer, model.max_syllables)
        if len(encoded["input_ids"]) > run["config"]["model"]["max_source_length"]:
            raise ValueError("MIDI template exceeds the checkpoint's source token limit")
        ...
        lyrics.append(tokenizer.decode(tokens[0], skip_special_tokens=True).strip())
    return lyrics
